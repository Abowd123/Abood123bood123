...
├── server.js       ← السيرفر (youtubei.js) - البحث + التفاصيل + التنزيل
├── package.json
└── .gitignore
```

كل الملفات في جذر المشروع مباشرة (بدون مجلدات فرعية)، حتى تعمل منصات
النشر (Railway، Render، إلخ) على أول محاولة دون الحاجة لتغيير أي
إعداد مثل "Root Directory".
...
```bash
# (لا حاجة، الملفات في الجذر مباشرة)
npm install
npm start
```
...
cd videosearch-pro && npm install
